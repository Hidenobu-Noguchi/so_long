...
/*   Updated: 2022/10/12 04:14:45 by hnoguchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_game_map_and_parse(int fd, t_game_info *game_info)
{
	...
	while (i < 1)
	{
		...
		{
			free(read_buf);
			map_frees(buf_game_map, game_info->map_info.height);
			exit_perror(" ");
		}
		while (game_info->map_info.height != 0 && i < game_info->map_info.height)
		{
			...
			{
				free(read_buf);
				map_frees(tmp_game_map, game_info->map_info.height);
				map_frees(buf_game_map, game_info->map_info.height);
...
int main(int argc, char **argv)
{
	...
	for (int j = 0; j <= i; j++)
	{
		free(game_info.map_info.game_map[i]);
	}
	// free(game_info.map_info.game_map);
	   // read map fd;
